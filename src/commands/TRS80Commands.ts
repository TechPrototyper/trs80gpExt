...
export class TRS80Commands {
  ...
  private async executeWithMode(isDebugMode: boolean): Promise<void> {
    try {
      ...
      if (!assemblyResult.outputFiles.bds) {
        vscode.window.showErrorMessage('No BDS file generated by assembler.');
        return;
      }
      if (!assemblyResult.outputFiles.cmd) {
        vscode.window.showErrorMessage('No CMD file generated by assembler.');
        return;
      }

      // Start emulator
      const modelArg = ConfigurationManager.getModelFlag(config.target);
  ...
  private async prepareEmulatorArgs(
    ...
    // Only user arguments, model argument is handled in EmulatorManager
